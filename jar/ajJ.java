public class ajj extends uI {
   private final uI a;
   protected String a;
   private static final JL[] a;
   private final ajm a = new ajm(this, new ajp());

   public ajj(uI var1) {
      this.a = var1;
   }

   public void b() {
      this.a = IK.a("of.options.qualityTitle");
      this.a.clear();

      for(int var1 = 0; var1 < a.length; ++var1) {
         JL var2 = a[var1];
         int var3 = this.c / 2 - 155 + var1 % 2 * 160;
         int var4 = this.d / 6 + 21 * (var1 / 2) - 12;
         if (!var2.a()) {
            this.a.add(new ajf(var2.a(), var3, var4, var2, this.a.a.a(var2)));
         } else {
            this.a.add(new ajg(this.a.a, var2.a(), var3, var4, var2));
         }
      }

      this.a.add(new tL(200, this.c / 2 - 100, this.d / 6 + 168 + 11, IK.a("gui.done")));
   }

   public void a(tL var1) {
      if (var1.a) {
         if (var1.e < 200 && var1 instanceof uy) {
            this.a.a.a((JL)((uy)var1).a(), 1);
            var1.a = this.a.a.a(JL.a(var1.e));
            this.a.a.b();
         }

         if (var1.e == 200) {
            this.a.a.b();
            this.a.a(this.a);
         }

         if (var1.e != JL.AA_LEVEL.ordinal()) {
            vk var2 = this.a.a();
            this.b(this.a, var2.getScaledWidth(), var2.getScaledHeight());
            this.a.a.b();
         }
      }

   }

   public void drawScreen(int var1, int var2, float var3) {
      this.drawDefaultBackground();
      this.a(this.a, this.a, this.c / 2, 15, 16777215);
      super.drawScreen(var1, var2, var3);
      this.a.a(var1, var2, this.a);
   }

   protected void keyTyped(char var1, int var2) {
      if (var2 == 1) {
         this.a.a.b();
      }

      super.keyTyped(var1, var2);
   }

   static {
      a = new JL[]{JL.MIPMAP_LEVELS, JL.MIPMAP_TYPE, JL.AF_LEVEL, JL.AA_LEVEL, JL.CLEAR_WATER, JL.BETTER_GRASS, JL.BETTER_SNOW, JL.CUSTOM_COLORS, JL.CONNECTED_TEXTURES, JL.NATURAL_TEXTURES, JL.CUSTOM_SKY, JL.CUSTOM_ENTITY_MODELS, JL.CUSTOM_GUIS, JL.EMISSIVE_TEXTURES};
   }
}
